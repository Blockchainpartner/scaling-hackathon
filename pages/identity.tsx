...
function NewUserProfile() {
  ...
  async function addUser() {
    const { data: registries } = await axios.get(
    ...
    if (res.status === 200) {
      addToast("New identity added. Validation needed", {
        appearance: "success",
      });
  ...
  return (
    <div
      className={`relative inset-0 ${
        open && !stopDemoDisclaimer ? "opacity-50 filter blur-sm" : null
      }`}
    >
      <div className="flex items-center xl:items-start flex-col xl:flex-row">
        ...
        <div className="flex-grow flex flex-col justify-between w-full xl:w-min xl:ml-8">
          ...
          <button onClick={addUser} className="btn-primary mt-8 py-6 w-full">
            {"SAVE IDENTITY"}
          </button>
          {true ? (
            <div
            ...
            >
              ...
              <p className="underline sm text-brand ml-2">
...
function ExistingUserProfil() {
  ...
  async function validateUser() {
    addToast("Please wait for the validation (~1-2mn)", {
      appearance: "info",
      autoDismiss: false,
    });
    const res = await axios.post(
      `http://localhost:8080/user/validate/${accountCtx.user.UUID}`
    );
    if (res.status === 200) {
      removeAllToasts();
        ...
        <div className="board w-full mb-8 xl:mb-0 xl:w-3/5 relative bg-white">
          <div>
            ...
            <div className="mt-10">
              <IdInfoItem
                label="Title"
                info={user.KYC.gender === "female" ? "Mrs" : "Mr"}
              />
              <IdInfoItem label="Name" info={user.KYC.name} />
              <IdInfoItem
                label="Date of birth"
                info={new Date(user.KYC.dob.date).toDateString()}
              />
...
const Identity = () => {
  ...
  // @ts-ignore
  const OpenLoginWindow = dynamic(() => import("../utils/openLogin"), {
    ssr: false,
  });
  const accountCtx = useAccount() as AccountCtx;
